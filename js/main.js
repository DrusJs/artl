...
/*
	...
	$('body').on("click", function(e){
		...
		if($("html").scrollTop() != 0){
				oldscrollfromtop = $("html").scrollTop();
		}
		console.log( e.target );

		function recheckwidgetclass(){
			
			
//		console.log("on click from top: " + oldscrollfromtop);
//		console.log("waited");
			if ($("html").hasClass("crm-widget-button-mobile") || ($(".b24-widget-button-shadow").hasClass("b24-widget-button-show"))) {
			
//		console.log("has class");
//				oldscrollfromtop = $("html").scrollTop();
				$("#b24widget_bg").fadeIn('fast', function(){
					$("html").css({ "height":"100vh", "overflow" : "hidden" }).scrollTop(0);
					$(".b24-widget-button-shadow").css({ "background":"rgba(13,34,68,0)"});//, "background-image": "url('/layout_main_only/images/widget_bg.png')", "background-position" : "center center", "background-repeat" : "no-repeat", "-webkit-backdrop-filter": "blur(12px)", "backdrop-filter": "blur(12px)" });
				});	
				
//				console.log("old scroll saved: " + oldscrollfromtop);
			}else{
//				console.log("has no class");
				$("html").css({ "height":"100%", "overflow" : "auto" });

//					console.log("check from top: " + oldscrollfromtop);
				if(oldscrollfromtop != 0){
//					$("html").scrollTop(oldscrollfromtop, function(){
						
					$('html, body').animate({scrollTop : oldscrollfromtop},0, function(){

						$("#b24widget_bg").fadeOut('fast');

						oldscrollfromtop = 0;
					});
//					console.log("unset from top: " + oldscrollfromtop);
				}	
			}
		 
		};
			window.setTimeout( recheckwidgetclass, 500 ); 
	...
	uphone = uphone.replace('+8','+7');
						var html = $.ajax({
							  ...
							  success: function(nucapa){
								  ...
								  if(nucapa != "0"){
									  $("#modal_form_screen_block_col1, #modal_form_screen_block_col2, #modal_form_screen_block_btn, #modal_form_screen_block_btn_na, #modal_form_screen_block_pca").slideUp("fast");
	...
	uphone = uphone.replace('+8','+7');
						var html = $.ajax({
							  ...
							  success: function(nucapa){
								  ...
								  if(nucapa != "0"){
									  $("#main_form_screen_block_col1, #main_form_screen_block_col2, #main_form_screen_block_btn, #main_form_screen_block_btn_na, #main_form_screen_block_pca").slideUp("fast");
	...
	uphone = uphone.replace('+8','+7');
						var html = $.ajax({
							  ...
							  success: function(nucapa){
								  ...
								  if(nucapa != "0"){
									  $("#main_prefooter_form_screen_block_col1, #main_prefooter_form_screen_block_col2, #main_prefooter_form_screen_block_btn, #main_prefooter_form_screen_block_pca").slideUp("fast");
